...
### Added
- **BLE heart-rate vitals pipeline (end-to-end).** The cheap, trustworthy contactless-vitals path the C6 radar couldn't deliver. `ruview-ble-scanner` crate: a pure, unit-tested Bluetooth-SIG Heart Rate Service (`0x180D` / `0x2A37`) decoder (bpm + RR-intervals→HRV + sensor-contact + energy; 10 tests incl. a real Polar H10 frame) plus a BlueZ GATT subscriber (`ruview-ble-hr-decoder`, unbonded, allowlist-by-MAC, auto-reconnect) and a passive advert observer, both → MQTT. New Pi REST bridge `scripts/pi/ruview-hrd.py` subscribes `ruview-ble/hr/#`, computes HRV (RMSSD), and serves `GET :3027/api/v1/hr` (+ `--fake` test mode). iOS `BLEHeartClient` + a Vitals HR card (BPM + HRV + contact chip) and a bonus Node-Health node; `fusedHeartRate` prefers the strap over the C6. Opt-in, consent-gated systemd units. Pairing runbook: `docs/ble-device-pairing.md`.
- **C6 (MR60BHA2) re-enabled as a vitals-only node behind a clutter trust gate.** `C6RadarClient.evaluateTrust` (near-field 0.2–1.5 m, HR physiological-band anchor, reject dead-constant clutter, 3 s trust-hold). The C6 never votes on presence/count; shown as the 4th device in Node Health.
- **Sensor catalog expanded** (`docs/ruview-sensor-catalog.html`): a **BLE devices** tier (HR strap/ring, pulse-ox, BP cuff, temp beacons, panic button), an **ML-grade raw-data radar** tier (TI IWR6843, Infineon BGT60), and a **professional/clinical** tier (Vayyar 4D imaging, Novelda/Xethru UWB, EarlySense/Emfit under-mattress).
- **Vayyar fall-detection pilot spec** (`docs/pilots/vayyar-fall-pilot.md`) — BOM + Orange Pi integration plan.
- **Radar ML model-selection research** (`docs/radar-ml-model-selection.md`) and the `ruview-featlog` data-flywheel logger (`scripts/pi/ruview-featlog.py`): logs the full fused feature vector (LD2450/LD2410/C6/audio, raw + gated) at 2 Hz to JSONL with a live label file, for training a precise on-device model.
...
### Fixed
- **Empty room no longer reads "occupied" — radar ghost gating (iOS).** Two static-reflector phantoms that inflated the count in an empty hall are now rejected: (1) LD2450 targets with nonphysical speed (`|v| > 350 cm/s` — an empty hall produced ±1190 cm/s ghosts) are dropped alongside the existing range gate; (2) LD2410C presence now gates on **energy** (moving ≥ 35% / still ≥ 20%) instead of its unreliable binary flags (which fired ~half the time at 0% energy off a 3.6 m wall reflection). Verified live: empty room → 0 across the board; a real walk-in still tracks cleanly. A resting person reads ~0 cm/s and full energy, so neither gate rejects a real occupant.
